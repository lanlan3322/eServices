...
<%
   ...
   if (DBDriver != null && !DBDriver.equals("")) {
       ...
       Log.println("[308] AuditLogin.jsp DBDriver: " + DBDriver);
   }
   ...
   if (foundUser && PersFile.validPassword(password) && (PersFile.isAuditor() || PersFile.isAdmin() || PersFile.isGLAdmin())) { 
...
<%   } else { %>
         ...
         <a href="<%= PersFile.getWebServer() %>/<%= PersFile.getWebFolder() %>/<%=company%>/pwdChangeAudit.html">Click here to change your password</a>
         </BODY></HTML>
<%   }
...
<%   Log.println("[420] AuditLogin.jsp invalid login attempt: " + email);
     ...
     Log.println("[420] AuditLogin.jsp File auditor: " + PersFile.isAuditor());
