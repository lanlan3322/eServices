...
<body>
<div align="right"><a href="javascript: void parent.contents.helpWindow('hrecoverfeed.html')"><span class="ExpenseLink">Screen Help?</span></a></div>
...
<form method="post" action="<%= PersFile.getAppServer() %>/<%= PersFile.getAppFolder() %>/RecoverPost.jsp">
...
<%    notbreakout = true;
      while (current_card <= total_cards) {  
      do { 
        status = DB.myResult.getString("STATUS").trim(); 
        ...
        reference = status+";"+cardtype + ";"+ Numbers[current_card].trim() + ";" +  ess.Utilities.getXMLString(ensureSpace(DB.myResult.getString("REFERENCE").trim())) + ";" + amount + ";" + recordCursor ; %>
          <tr>
          ...
          <td width="13%" <%=backcolor%> align="center"><%= DD.getSimpleDate(DD.adjustEpoch(DD.getDateFromXBase(DB.myResult.getString("TDATE"))))%></td>
          <td width="19%" <%=backcolor%> align="center"><%= amount %></td>
          <td width="50%" <%=backcolor%> align="center"><%= DB.myResult.getString("INFO")%></td>
          </tr>
     <% newbackcolor = backcolor;
      ...
      do {
        ...
        if (!Cards[current_card].trim().equals("") && !Numbers[current_card].trim().equals("") && DB.setResultSet( DB.SQLReplace( DB.SQLReplace(selectItems,"$card$",Cards[current_card]) , "$number$",Numbers[current_card]) )) {
          notbreakout = false;  //probably useless    
        } else {
          notbreakout = true;
        }
      } while(current_card <= total_cards); 
...
<% } else { %>
	...
	<body>
	<p><h1> Sorry <%=request.getParameter("email")%>.  There are no prepopulated items for <%=Reporter.getEmailAddress()%> to recover.
	</h1></p>
