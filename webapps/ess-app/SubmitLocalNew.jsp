...
String securityContext = config2.getInitParameter("ESSSecurity");  
...
if (securityContext.toUpperCaseEquals("HOST")
{
   loginClock = request.getHeader("sm-user");
   pFlag = PersFile.setPersNumInfo(loginClock); 
} else {
   password = request.getParameter("password");
   if (password == null) password = "";
   pFlag = PersFile.setPersInfo(ownersName); 
   if(pFlag) pFlag = PersFile.validPassword(password); 
}
if (pFlag) pFlage = email.equalsIgnoreCase(PersFile.getEmailAddress);  //Do I want this or do I want the Pers_num??
...
Log.println("[000] SubmitLocal.jsp pvoucher = " + xref);
   ...
   //Need to put a check here to check up on the personnel number...or...look into the processReport to see if we can 
   //deal with it there.
