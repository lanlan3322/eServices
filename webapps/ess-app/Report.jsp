...
<frameset framespacing="0" border="false" frameborder="0" rows="70,*" onLoad="SetCommOK()"  onUnLoad="SetLogoutOK()">
  <frame name="banner" scrolling="no" noresize src="<%= PersFile.getWebServer() %>/<%= essWebFolder %>/<%= essCustomFolder %>/bannerLegacy.html" target="contents">
  <frameset cols="160,*,1">
    ...
    <frame name="main" src="<%= PersFile.getAppServer() %>/<%= essAppFolder %>/Intro.jsp?email=<%=email%>&database=<%=database%>&ccode=<%=PersFile.getChallengeCode()%>">
