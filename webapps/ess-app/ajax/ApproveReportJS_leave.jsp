...
function initForm() {
  ...
  document.forms[formStartNumber].password.value = "no";
  document.forms[formStartNumber].msgdata.value = "";
      ...
      if (!xfound) {
        ...
        if (msgdataRaw.value.length < 1) {
           ...
           if (confirm("<%=Lang.getString("APP_PROCEED")%>")) {
             resolveMsgData();
             postSimpleForm(parent.defaultApps + 'ajax/ApproveSave_leave.jsp',document.forms[formStartNumber]);
             submitSafetyFlag = false;
           }
        }
