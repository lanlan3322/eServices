...
Log.println("[000] UserPersNum.jsp accessed by :" + ownersName);
...
<%
   ...
   if (lastName != null) {
      ...
      Log.println("[000] UserPersNum.jsp SQL:" + SQLCommand);
   } else {
      SQLCommand = null;
   }   
   ...
   if (SQLCommand != null && Reg.setResultSet(SQLCommand)) { %>
     ...
     <body>
     <h1><%= Lang.getString("personnelNumSel")%></h1><h2>
<%   
     String Pers_num = ""; 
     String Lname = "";
     String Fname = "";
...
<%     
     ...
     %>          
            ...
            <td width="20%" <%=backcolor%>><a href="javascript: void selectUser('<%= Pers_num%>')"><span class="ExpenseReturnLink"><%= Lang.getString("select")%></span></a></td>
            </tr>
    ...
    Log.println("[500] UserPersNum.jsp java.lang exception");
%>
...
function FormInit(){  // This function is never called!!!
  window.focus();
...
<% } else { %>
    ...
    <% Log.println("[400] UserPersNum.jsp Selection only.");
    } //if (Reg.setResultSet(SQLCommand)) 
%>
  <form method="POST" action="UserPersNum.jsp" onSubmit="return SubmitSearch()">
