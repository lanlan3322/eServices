...
<% 
   ...
   String OPERATION_ID  = request.getParameter("voucher");
...
if (pFlag && PersFile.getChallengeCode().equals(CCode)) { 
   ...
   String SQLCommand_Leave = "SELECT * FROM DB_OPERATION WHERE OPERATION_ID =";
	SQLCommand_Leave += OPERATION_ID + PersFile.getSQLTerminator();
%>
     <h1>Inventory Request Details <%=OPERATION_ID%></h1>
...
%>
     ...
     <thead>
         <tr>
             ...
             <th class="ExpenseTag" width="10%" <%=backcolor%>><u>Name</u></td>
             <th class="ExpenseTag" width="10%" <%=backcolor%>><u>Date</u></td>
             <th class="ExpenseTag" width="50%" <%=backcolor%>><u>Reason</u></td>
             <th class="ExpenseTag" width="10%" <%=backcolor%>><u>Status</u></td>
             <th class="ExpenseTag" width="5%" <%=backcolor%>><u>Type</u></td>
         </tr>
     </thead>
<%
		...
		reference = OPERATION_BY;

     %>          
            <tr>
			<td width="5%">&nbsp;</td>
            <td width="10%" bgcolor="#42c5f4" align="center"><%= OPERATION_BY%></td>
            <td width="10%" bgcolor="#42c5f4" align="center"><%= OPERATION_CREATED%></td>
            <td width="50%" bgcolor="#42c5f4" align="center"><%= OPERATION_REASON%></td>
            <td width="1%"  bgcolor="#42c5f4" align="center"><%= OPERATION_STATUS%></td>
            <td width="5%" bgcolor="#42c5f4" align="center"><%= OPERATION_TYPE%></td>
            <td width="5%" bgcolor="#42c5f4"></td>
            </tr>
<%
			String SQLCommand2 = "SELECT DB_ITEM.ITEM_NAME, DB_ITEM.ITEM_CATEGORY, DB_ITEM.ITEM_DESC, DB_OPERATED_ITEM.REF_AMOUNT_ITEM FROM DB_ITEM JOIN DB_OPERATED_ITEM ON DB_ITEM.ITEM_ID = DB_OPERATED_ITEM.REF_ID_ITEM WHERE TRIM(DB_OPERATED_ITEM.REF_ID_OPERATION) = " + OPERATION_ID;
...
<table id="approveSection">
    <form method="POST" accept-charset="ISO-8859-1" action="<%= PersFile.getAppServer() %>/<%= PersFile.getAppFolder() %>/inventorySave.jsp">
    ...
    <tr class="col1of2">
		<td>
		<input type="button" id="btApprove" value="Reject this request with the above comments" name="B2" onClick="Javascript: void Approve()">
		</td>
	</tr>
    </form>
</table>
...
function initForm() {
  ...
  document.forms[0].reference.value = "<%= OPERATION_BY %>";
  document.forms[0].voucher.value = "<%= OPERATION_ID %>";
