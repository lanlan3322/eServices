...
<%
   ...
   String screenname = "userprimary_add_on";//request.getParameter("screenname");
...
%>
     ...
     <h1>Add-on operation for users:</h1><h2>
<%   
...
function x(AddValue) {
  if (AddValue.length > 0) {
    ...
    alert("System is not able to fulfill this request.\nPlease try again.\nIf problem persists, contact support.");
      ...
      <form method="POST" action="<%= PersFile.getAppServer() %>/<%= PersFile.getAppFolder() %>/edit/AddOnConfirm_leave.jsp">
		...
		<table>
		<tr>
			<td align="right">Leave Types:</td> 
			<td class="expenseType"><select name="leavetype" xref="leavetype" class="limited-width" onMouseDown="if(document.all) this.className='expanded-width';" onBlur="if(document.all) this.className='limited-width';" onChange="if(document.all) this.className='limited-width'"></select></td>
		</tr>
		<tr>
		...
		<p><input type="button" value="Apply" name="B1"onClick="Javascript: void Apply()">&nbsp; <span class="ExpenseTag">Click &quot;Apply&quot; to offset indicated days of leave for checked users.(1: add one day; -1:deduct one day)</span></p>
      </form>
      ...
      function initForm() {
        if (parent.contents.getDBValue) {
			if(document.forms[1].leavetype){
				parent.contents.setListWithValue(document.forms[1].leavetype,parent.contents.getLeaveTypes(),0,1,true,true,1);
				document.forms[1].leavetype.options[0].text = "Add-on"; 
				document.forms[1].leavetype.options[0].value = "Add-on";
				document.forms[1].leavetype.selectedIndex = 0;
			}
      ...
      function onCheckAll(obj){
        for (var i = 0; i < document.forms[0].length; i++) {
          //if (isIE) {
             if (document.forms[0].elements[i].name == "select_this_item") {
				 if(obj.checked){
      ...
      function Apply(){
        var delim = "";
		document.forms[1].type.value = document.forms[1].leavetype.options[document.forms[1].leavetype.selectedIndex].text;
		document.forms[1].reference.value = "";
        var isIE = false;
        ...
        if (delim == ";") {
          if (confirm("Indicated leave change will be applied to selected users.  Is it 'OK' to proceed?")) {
            document.forms[1].submit();
			//alert(document.forms[1].reference.value);
          } else {
            document.forms[1].reference = "";
          }
        } else {
          alert("Must check item(s) that you wish to apply");
        }
      }
