...
<%
   ...
   String screenname = "userprimary_nominee";//request.getParameter("screenname");
...
%>
     ...
     <%
            ...
            { 
			  ...
			  String titleNom = Reg.getTrim(columnName);
			  if(Reg.getTrim(columnName).equalsIgnoreCase("BILLEXP")){
				  titleNom = "ACTIVE NOMINEE";
			  }
     %>     <td><u><%= titleNom %></u></td>
     <%
            }
...
//Do the retrieve from the result set here - below is the row
            ...
            { 
			  ...
			  if(columnName.equalsIgnoreCase("BILLEXP")){
				if(Reg.myResult.getString(columnName).equalsIgnoreCase("1")){
					nominee = true;
				}
			  }
            }
     ...
     %>     
			<tr bgcolor="#6495ED">
            <td width="5%"><input type="checkbox" name="select_this_item" value="<%= getKeyDocument(Reg, Log, keyList,sqlTableName) %>"></td>
     <%
            for (int i = 0; i < columns.size(); i++)
            { 
              columnElement = (org.jdom.Element) columns.get(i);
...
function x(AddValue) {
  if (AddValue.length > 0) {
    ...
    alert("System is not able to fulfill this request.\nPlease try again.\nIf problem persists, contact support.");
      ...
      <form method="POST" action="<%= PersFile.getAppServer() %>/<%= PersFile.getAppFolder() %>/edit/inventoryAssign.jsp">
      ...
      <p><input type="button" value="Assign" name="B1"onClick="Javascript: void Assign(1)">&nbsp; <span class="ExpenseTag">Click &quot;Assign&quot; to nominate checked person for stock checking</span></p>
      <!--
	  <p><input type="button" value="Remove" name="B1"onClick="Javascript: void Assign(0)">&nbsp; <span class="ExpenseTag">Click &quot;Remove&quot; to remove checked person from stock checking nominees' list</span></p>
      -->
	  </form>
      ...
      function Assign(action){
        ...
        document.forms[1].xaction.value = action;
        if (delim == ";") {
          if (confirm("Checked person(s) will be assigned for stock checking.  Is it 'OK' to proceed?")) {
            document.forms[1].submit();
          } else {
            document.forms[1].reference = "";
          }
        } else {
          alert("Must check person(s) that you wish to assign");
        }
      }
