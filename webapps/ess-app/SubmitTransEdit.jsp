...
<%
   ...
   String[] encrypts = new String[0];
   String screenName = request.getParameter("screenname");
   String xaction = request.getParameter("xaction");

   Logfile.println("[000] SubmitEdit.jsp screen name: " + screenName + ", xaction: " + xaction ); 
...
//          3. Doesn't exist as a field in a table
   ...
   if (!checks.isEmpty())
   {
      for (int j = 0; j < checks.size(); j++)
      {
          ...
          Logfile.println("[000] SubmitEdit.jsp method: " + chk_method + ", returns: " + chk_returns);
Logfile.println("[000] SubmitEdit.jsp chk_allowblank: " + chk_allowblank + ", value: " + db.getSQLArrayValue(values, chk_allowblank));

          blankReturn = db.getSQLArrayValue(values, chk_allowblank); 
          if (blankReturn == null || !blankReturn.equals(""))
          {
Logfile.println("[000] SubmitEdit.jsp -checking");
             if (chk_method.equalsIgnoreCase("BOTH") || xaction.equalsIgnoreCase(chk_method))
             {
                chk_sql = db.SQLReplaceWArray(chk_sql, values, "$", "$");
                if (db.setResultSet(chk_sql))
                {
...
<p align="center"><a href="javascript: void parent.frames[1].PersWindow('<%= PersFile.getWebServer() %>/<%= PersFile.getWebFolder() %>/edit/<%= editScreenName %>')" tabindex="1"><em><strong>New Edit Screen</strong></em></a></p>
...
// put the check else here and the failure logic
     ...
     Logfile.println("[400] SubmitEdit.jsp security object removed for " + ownersName); %>
