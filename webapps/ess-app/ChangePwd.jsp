<%--
ChangePwd.jsp - Changes a users password
...
<%
   ...
   String loginScreen;
   if (changeScreen.equals("pwdChangeAudit.html")) 
   {
	    loginScreen = "audit/AuditLogin.jsp";
   } else {
	    loginScreen = "Login.jsp";
   }
...
<%
   ...
   if (PersFile.setPersInfo(email)) {
     Log.println("[000] ChangePwd.jsp primary login: " + email);
      foundUser = true;
   } else {
     Log.println("[000] ChangePwd.jsp secondary login attempt: " + email2);
     if (email2 != null) foundUser = PersFile.setPersInfo(email2);
     email = email2; 
   }

   if (foundUser && PersFile.validPassword(password)) { %>
...
<%   if (newPwd1.equals(newPwd2) && newPwd1.length() >= minimumPwdLength) {
       ...
       SQLCommand = Reg.SQLReplace(SQLCommand,"$email$",Reg.repStr(email.toUpperCase()));
       ...
       if (xReturn == 1) { 
         Log.println("[240] ChangePwd.jsp succeeded: " + email);
         ...
         <form method="POST" action="<%= PersFile.getAppServer() %>/<%= PersFile.getAppFolder() %>/<%= loginScreen %>">
...
<%         
         if (xReturn == -1) {   
            Log.println("[500] ChangePwd.jsp update failure (SQL/access error): " + email); 
%>
            Update process failed (access error) - please try again.  If problem persists please contact support.<br><br>

<%       } else if (xReturn > 1) {
            Log.println("[400] ChangePwd.jsp update failure (Multiple records found): " + email);
%>
            Update problem: Multiple user records have been changed.<br><br>

<%       } else { 
            Log.println("[500] ChangePwd.jsp update failure (Bizarre error): " + email);
%>
...
<%       Log.println("[400] ChangePwd.jsp mismatch failure: " + email); %>
       <HTML><BODY>
...
<%       Log.println("[400] ChangePwd.jsp user not found: " + email); %>
       <HTML><BODY>
