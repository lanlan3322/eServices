...
//deals with comment required for multi person transaction (above line is true)
   ...
   if (commentFld != null && commentFld.value.length < parent.contents.getCommentLen()) {
     if (commentFld.value.length == 0) {
       commentFld.value = "See attendee list";
     } else {
       commentFld.value = commentFld.value + " - see attendee list";
     }  
   }
}
...
function setPeopleList(listType, keyName, keyValue, arrayString) {
   ...
   parent.contents.setDBPair(parent.PersDBase,listType,x);
}