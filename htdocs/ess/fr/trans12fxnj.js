...
function FillForm() {
Log.println("trans12fxnj.js","FillForm");
...
//    parent.setList(document.forms[formStartNumber].expense_2_expensetype, expArray.concat(parent.getInclude("hotel")),parent.getLodgingExc() + ";");
    ...
    alert("Vous devez d'abord spécifier une description puis choisir Reçu d'hôtel à nouveau.");
    parent.ListMemory();
  }
}
...
Log.println("trans12fxnj.js","SubmitRec");
  ...
  with (document.forms[formStartNumber]) {
   ...
   if ((xPost == "Post") && CheckRoomAmount(amount)
      ...
      if (CheckTotals()) {
        ...
        //parent.ListDelay();  // removed jh 2012-06-28
      } else {
        alert("La facture d'hôtel ne balance pas - Veuillez la revoir.");  //12/13/02
        retVal = false;
      }
    } else {
      if (xPost = "Post") {
  ...
  with (document.forms[formStartNumber]) {
    if (expense_1_persplit.value.length == 1 && checkString.indexOf(expense_1_persplit.value) > -1) {
        if (expense_1_persplit.value != "1" && expense_1_comment.value.length < 5) {
           alert("Si plus d'une personne - entrer le nom des individus dans la section Description !");
           retVal = false;
           expense_1_persplit.focus();
        }
    } else {
        alert("Le nombre de personnes doit être un nombre entier entre 1 et 8!");
        retVal = false;
        expense_1_persplit.focus();
    }
  }
...
function CheckTotals(formObj) {
   ...
   if (recAmt - expAmt < 0.005 && expAmt - recAmt < 0.005) {
     if (exp1Amt >= 0) {    // was just > (allows zero hotel items through - parameterize)
        ...
        alert("Le montant de la chambre doit être plus grand que zéro")
        return false;
     }
   } else {
     alert("Reçu non balancé par " + (recAmt - (exp1Amt + exp2Amt)));
     return false;
   } 
}
...
function CheckDedicated(expObj, chgObj) {
  ...
  if (dedicated != "" && y != dedicated) { 
    retVal = false;
    alert("Méthode de paiement non valide " + y + " pour type de dépense " + x + ". Utilisez " + dedicated + ".");
  } else {
    if (y != dedicated && parent.getDBValue(parent.getDedicatedMethod("1"), y,0,1) > "") {
      retVal = false;
      alert("Méthode de paiement " + y + " Ne peut être utilisé pour type de dépense " + x + ".");
    }
  }  
  return retVal;
}  
...
function CheckRoomAmount(tag) {
  if (isNaN(tag.value)) {
     alert("La donnée dans le champ Montant n'est pas numérique.");
     tag.focus();
     tag.select();
     return false;
  } else {
...
function CheckExpType(object) {
  if (object.selectedIndex == 0) {
     alert("Un type de dépense est requis.");
     object.focus();
     return false;
  }  else {
          return true;
...
function CheckComment(tag,expensetype) {
  ...
  if (StrV != null && StrV != "" && NeedComment.indexOf(";" + StrV + ";") > -1 && Check.length < CmtLen) {
     alert("Description requise d'au moins " + CmtLen + " caractères");
     tag.focus();
     return false;
  } else {
     return true;
...
function addExpense() {
 with (document.forms[formStartNumber]) { 
  if (CheckAmount(expense_2_amount, expense_2_expensetype.options[expense_2_expensetype.selectedIndex].value)
    ...
    if (expense_2_attendeelist.value == null) alert("Liste des participants incorrecte\ncontactez le support");//expense_2_attendeelist.value = "[]";
...
function launchAttendees() {
 with (document.forms[formStartNumber]) {
  if (CheckAmount(expense_2_amount)
    ...
    alert("Vous devez avoir des informations de dépenses valides avant d'ajouter des participants.\nVeuillez fournir l'information manquante !");
  }
 }
}

function screenLoad() {
Log.println("trans12fxnj.js","screenLoad");
  FillForm();
  return true;
}
...
Log.println("trans12fxnj.js","screenUnload " + cancelThisRequest );
  var retVal; 
  ...
  cancelThisRequest = "";
  Log.println("trans12fxnj.js","screenUnload complete");
  return retVal;
}
