// shared2.js - list handling and formatting routines
...
function setList (SelectObj, ListArray, SkipString) {
//Used to load list options from a simple array
  ...
  if (SelectObj.length == 0) {
    ...
    for (var i = 0; i < ListArray.length; i++) {
      if (ListArray[i] == "" || SkipString.indexOf(";" + ListArray[i] + ";") == -1) SelectObj.options[j++].text = ListArray[i];
    }
...
function setDateObj(DateString, df) {    
  var xSep = "/";
  if (df == null) df = parent.dateFormat;
  if (df.indexOf("-") > -1) xSep = "-"; 
  var xFormat = [["MM",df.indexOf("MM")],["dd",df.indexOf("dd")],["yy",df.indexOf("yyyy")]];
  xFormat.sort(function(a,b) {return a[1] - b[1]});
  var DateObj = new Date();
  var i = DateString.indexOf(xSep);
  var j = DateString.indexOf(xSep,i+1);
  var xValue = new Array(3);
  xValue[0] = parseInt(DateString.substring(0,i));  //Can work on this later to make month-day input OK.
  xValue[1] = parseInt(DateString.substring(i+1,j));    //by checking on the lack of a yyyy 
  xValue[2] = parseInt(DateString.substring(j+1));
  for (var k = 0; k < 3; k++) {
    var x = xValue[k];
    var z = xFormat[k][0];
    if (z == "MM") {
       DateObj.setMonth(x - 1);
    } else if (z == "dd") {
       DateObj.setDate(x);
    } else if (z == "yy") {
      if (x < 100) {
...
function setListWithValue (SelectObj, ListArray, x, y, both) {
//Used to load list options from a array with name,value pair [[x0,y0],[x1,y1]...[xn,yn]]
  ...
  if (SelectObj.length == 0) {
    ...
    for (var i = 0; i < ListArray.length; i++) {
      if (both) {
        if (ListArray[i][y] > "") {
           SelectObj.options[i].text = ListArray[i][x] + " (" + ListArray[i][y] + ")";
        } else {
      ...
      SelectObj.options[i].value = ListArray[i][y];
    }    
  SelectObj.selectedIndex = 0;
  } 
}

function setXLationWithValue (SelectObj, ListArray, x, y, both) {
//Used to load list options from a array with name,value pair [[x0,y0],[x1,y1]...[xn,yn]]
  if (x == null) x = 0;
  if (y == null) y = 1;
  if (both == null) both = false;
  if (SelectObj.length == 0) {
    ...
    for (var i = 0; i < ListArray.length; i++) {
      if (both) {
        if (ListArray[i][y] > "") {
           SelectObj.options[i].text = getJSX(ListArray[i][x]) + " (" + ListArray[i][y] + ")";
        } else {
           SelectObj.options[i].text = getJSX(ListArray[i][x]);
        }
      } else {
        SelectObj.options[i].text = getJSX(ListArray[i][x]);
      }
...
//Used to load list options from a array with name = name1 + ': ' + name2 and then name1 (key) as value[[x0,y0],[x1,y1]...[xn,yn]]
...
function setArray2Str(ListArray) {
   var delimit = "";
   var xStr = O 
   for (var k = 0; k < ListArray.length; k++) {            
      if (typeof(ListArray[k]) == "string") {
        xStr += delimit + Q + correctStr(ListArray[k]) + Q;
      } else {
        xStr += delimit + setArray2Str(ListArray[k]);
      }
      delimit = D;
   }              
   xStr += C;      
...
function getTotalSubListValue(WhichList, SubListName, ItemName, EditName, EditValueStr) { 
  ...
  for (var i = 0; i < WhichList.length; i++) {
     ...
     if (subList == null) alert("error - Sublist null in getTotalSubListValue");
...
var generalLimit;
function checkAmtFldOK(tag, xType) {
  ...
  var amount = new oNumber(tag.value);
  if (isNaN(amount.standard) || rtrim(amount.standard) == "") {
     alert("Data in the amount field is not a valid numeric value");
     tag.focus();
     return false;
  } else {
    var Check = amount.parseFloat();
    if (Check < .01 && !getAllowNegative()) {
      alert("Amount field must be greater than zero");
      tag.focus();
      return false;
    } else if (Check < .01 && xType != null && (PerReq.indexOf(xType) > -1)) {
      alert("Personal items must be greater than zero");
      tag.focus();
      return false;
    } else if (Check >= generalLimit) {
      alert("Amount field must be less than " + generalLimit);
      tag.focus();
      return false;
    } else {
      return true;
...
function getReportCurrencyLabel() {
   return getCurrencyLabel(getNameValue(parent.contents.Header,"CURRENCY"));
}

function getCurrencyLabel(retVal) {
   if (retVal == null) retVal = getNameValue(parent.contents.Header,"currency");
...
function getDistanceLabel() {
   var retVal;
//   var ML = parent.contents.getMileageList();
//   if (parent.MList) {
//     retVal = getNameValue(parent.MList[0][1],"units");
//   if (ML.length > 0) {
//     retVal = getNameValue(ML[0][1],"units");
//   } else {
     retVal = getDistanceUnit();
//   }
