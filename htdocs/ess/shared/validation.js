...
function amountVal(textObj) {
        ...
        if(newValue.length != "" && newValue != "0" + parent.decimal + "00") {
            if (newValue != "NaN") {
                moneyFormat(textObj);
                return true;
            } else {
...
function isRate(textObj) {
        ...
        for(var i = 0; i != newLength; i++) {
                aChar = newValue.substring(i,i+1);
                if((aChar < "0" || aChar > "9") && aChar != parent.decimal && aChar != "-") {
                        intAlert(textObj);
                        return false;
                }
        }
...
function isCurrency(tag, isMoney) {
  ...
  if (isNaN(tag.value)) {
    ...
    var amount = new parent.oNumber(tag.value);
...
function setFlags(textObj) {
  NV = parent.ltrim(parent.rtrim(textObj.value));  
...
function checkDepartAJAX(depart, obj2, obj3, objList4) {
  ...
  httpXMLDepart = parent.GetXmlHttpObject();
  parent.getInfo(httpXMLDepart, parent.defaultApps + 'Check.jsp?check=depart&param1=' + encodeURIComponent(depart) + '&param2=' + encodeURIComponent(companyStr), stateDepart, true);
}
