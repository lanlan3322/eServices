...
function setLocalObjCurrency(LocalValue) {
   ...
   parent.makeCurrency(LocalXFldObj);
}
function setFgnObjAmount(FgnValue) {
   FgnFldObj.value = FgnValue;
   if (FgnValue != "") parent.makeCurrency(FgnFldObj);
}   
...
function setFXFormObj(x) {
   fxFormObj = x;
}
function getFXFormObj() {
   return fxFormObj;
}
function setFGN(FGNArray, xRecamt, yAmount) {
  ...
  for (var i = 0; i < FGNArray.length; i++) {
     fxFormObj.elements[FGNArray[i][0]].value = FGNArray[i][1];
     if (FGNArray[i][0] == "units" && FGNArray[i][1] == "") {
       fgnFlag = false;
       if (!isEmpty(fxFormObj.elements[yAmount])) fxFormObj.elements[yAmount].readOnly = false;
     }
  }
  FXConvert(xRecamt, yAmount, true);
  if (fxFormObj.elements[FXFormNames[1]].value != "") setFgnProtected(false);
  if (typeof(setMultiExpenseFromFX) == "function") setMultiExpenseFromFX();   //Can I check for the obj here and leave as fx1
}

function setFgnProtected(fgnFlag, xRecamt, yAmount, thisFormObj) {
  //true = fgn readonly   false = amount readonly
  if (thisFormObj == null) thisFormObj = fxFormObj;
  if (isEmpty(xRecamt)) xRecamt = FXFormNames[5];  //recamt
  if (isEmpty(yAmount)) yAmount = FXFormNames[6];  //amount
  thisFormObj.elements[xRecamt].readOnly = fgnFlag;
  thisFormObj.elements[yAmount].readOnly = !fgnFlag; 
}
...
function __FXConvert(xRecamt, yAmount, doNotShowMsg, thisForm) {
  if (isEmpty(xRecamt)) xRecamt = FXFormNames[5];  //recamt
  if (isEmpty(yAmount)) yAmount = FXFormNames[6];  //amount
  if (thisForm == null) thisForm = fxFormObj;
  if (!isEmpty(thisForm.elements[xRecamt]) && !isEmpty(thisForm.elements[yAmount])) {
    ...
    alert("rate: " + thisForm.elements[FXFormNames[1]].value);
    alert("recamt: " + thisForm.elements[xRecamt].value);
      if (thisForm.elements[FXFormNames[2]].value == "0") { //ratetype   was 1
...
function defCurrency(thisForm){
  if (thisForm == null) thisForm = fxFormObj;
  var ResultItem = parent.getDBSingle(parent.WorkDBase,"last_currency");
  if (ResultItem[0] == null) ResultItem[0] = "";
  ResultItem[0] = parent.rtrim(ResultItem[0]);
  if (ResultItem.length > 0 && ResultItem[0].length > 0) {
    var ResultSet = parent.getDBSingle(parent.PersDBase,"currency","units",ResultItem[0]);
...
function CheckFGNAmount(objRecamt, thisForm) {
  var retVal = false;
  if (thisForm == null) thisForm = fxFormObj;
