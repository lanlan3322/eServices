...
function initForm() {
	document.forms[formStartNumber].action = parent.defaultApps + "ajax/receipts/Upload_leave.jsp";
...
function getRemoveFiles(x) {
   ...
   var ParamString = "?pvoucher=" + parent.myESSMenu.voucher;//x;
...
function loadHTMLAJAX(params) {
  ...
  var LoadJSP = parent.appServer + "/" + parent.appFolder + "/ajax/receipts/ReceiptScans_leave.jsp" + params + "&force=" + Math.random();
...
function SubmitLeave(bAttachmentRequired){
	var sAttachments = document.getElementById("scanFiles").innerHTML;
	var bNotAttached = sAttachments.indexOf("img")>=0?false:true;
	if(bAttachmentRequired && bNotAttached){
		alert("Attachemnt is required!");
	}
	else{
		parent.loadHTMLScreenAJAX('submitWithGuide_leave.html');
	}
}
