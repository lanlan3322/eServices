...
function Logout(logoutProgram) {
  if (logoutProgram == null) logoutProgram = defaultApps + "Logout.jsp?email="+ parent.Header[8][1];
  ...
  SetCookie("AdisoftHead","");
  SetCookie("AdisoftPurp","");
  SetCookie("AdisoftRcpt","");
...
function Newie(ScreenHTML) {
  if (HeadList.length > 0) {
    ...
    parent.main.document.write("<li><p align=\"left\"><a href=\"javascript: void parent.contents.TransWindow('" + ScreenHTML + "')\">Return - Continue working in current report.</a></p></li>\n");
    if (!ReportIsSaved) {
      ...
      switch (TheForm.elements[ListBuffer[i][$name$]].type) {
        ...
        case "select-one":
           if (TheForm.elements[ListBuffer[i][$name$]].subtype == "list") {
             ...
             j = 0;
             z = true;
//           if ( !TheForm.elements[ListBuffer[i][$name$]].xref ) {   //need to change this
 ...
 if (dupFlagOK) { 
  ...
  if (genNoError() && cusNoError()) {
    ...
    for (var i = 0; i < TheForm.length; i++) {
      ...
      if (TheForm.elements[i].subtype){
        SubType = TheForm.elements[i].subtype;
      } else {
        SubType = TheForm.elements[i].type;
      } 
      switch (SubType) {
        case "text":      
...
function NextXref(xItem, nPackZeros) {
  ...
  var NextRef;
  if (HeadList.length == 0) {
    NextRef = 1;    //was zero before
  } else {
    NextRef = parseInt(getItemValue(HeadList[HeadList.length-1],xItem)) + 1;
  }
  if (nPackZeros != null) NextRef = zeroPack(NextRef,nPackZeros);  //after Kohler install add a check for purpose dup possibility
...
var backcolor = "class=\"offsetColor\"";  //These are used for alternating a colored line with a white one
var oldbackcolor = "";
var newbackcolor; 
...
//this is normally called after most actions to show the used the current status of his report arrays.
  ...
  top.frames[2].bList.document.open("text/html");
  top.frames[2].bList.document.write("<HTML><HEAD><link rel=\"stylesheet\" href=\"" + defaultHome + "expense.css\" type=\"text/css\"></HEAD><BODY>\n");
  if (TailList.length == 0) {
    top.frames[2].bList.document.write("<h2>&nbsp;No receipts entered yet!</h2><BR><BR>");
  } else {
    backcolor = "class=\"offsetColor\"";  
    oldbackcolor = "";
    top.frames[2].bList.document.write("<form><table border=\"0\" cellspacing=\"0\" cellpadding=\"3\" width=\"90%\" bordercolordark=\"#008080\">\n");
    PrintTailList();
    top.frames[2].bList.document.write("</table></form><BR>\n");
  }
  top.frames[2].bList.document.write("</BODY></HTML>");
  top.frames[2].bList.document.close();
}
...
function PrintHeadList() {
  ...
  for (var j = 0; j < HeadList.length; j++){
    PrintItems(HeadList[j],"HeadList",j,cA,fA,getHLSubLines(),defaultHead);
  }
}
...
function PrintItems(trans,list,index,cA,fA,subLines,defaultHTML){
  if (!subLines) {
     var subLines = Array();
  }
  var SubText = "";
  ...
  parent.main.bList.document.write("<tr>\n");
  parent.main.bList.document.write("<td width=\"10%\" " + fA[0] + " " + backcolor +">" + FlatCell(trans,cA[0],index) + "</td>\n");
  parent.main.bList.document.write("<td width=\"15%\" " + fA[1] + " " + backcolor +">" + FlatCell(trans,cA[1],index) + "</td>\n");
  parent.main.bList.document.write("<td width=\"15%\" " + fA[2] + " " + backcolor +">" + FlatCell(trans,cA[2],index) + "</td>\n");
  parent.main.bList.document.write("<td width=\"40%\" " + fA[3] + " " + backcolor +">" + outstring +"</td>\n");
  if (link1 == "" || !getEditable()) {
    parent.main.bList.document.write("<td width=\"10%\" " + backcolor +"></td>\n");
    parent.main.bList.document.write("<td width=\"10%\" " + backcolor +"></td>\n");
  } else {
    parent.main.bList.document.write("<td width=\"10%\" " + backcolor +"><a href=\"javascript:void " + link1 + "\"><span class='ExpenseReturnLink'>edit</span></a></td>\n");
    parent.main.bList.document.write("<td width=\"10%\" " + backcolor +"><a href=\"javascript:void " + link2 + "\"><span class='ExpenseReturnLink'>delete</span></a></td>\n");
  }
  parent.main.bList.document.write("</tr>\n");
...
function GetLocal() {
 ...
 populateReport(GetCookie("AdisoftHead"), GetCookie("AdisoftPurp"), GetCookie("AdisoftRcpt"), msg);
 ReportIsSaved = false;
}
...
function CreateCookies() {
  ...
  SetCookie("AdisoftHead",setArray2Str(Header));  //was outstring
  SetCookie("AdisoftPurp",setArray2Str(HeadList));
  SetCookie("AdisoftRcpt",setArray2Str(TailList));
}
